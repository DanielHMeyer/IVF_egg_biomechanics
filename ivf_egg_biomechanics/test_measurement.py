...
class TestMeasurement(unittest.TestCase):
    ...
    def test_init_function(self):
        ...
        self.assertEqual(meas.measurement_data[m.PatientKeys.PATIENT_NUMBER.value], 1234)
        self.assertEqual(meas.measurement_data[m.PatientKeys.PATIENT_AGE.value], 24)
        self.assertEqual(meas.measurement_data[m.PatientKeys.MATURE_OOCYTES.value], 14)
        self.assertEqual(meas.measurement_data[m.PatientKeys.CLINIC.value], 'STANFORD')
        self.assertEqual(meas.measurement_data[m.PatientKeys.OOCYTE_NUMBER.value], 5)
        self.assertEqual(meas.measurement_data[m.PatientKeys.POSITION.value], 6)
        self.assertEqual(meas.measurement_data[m.OutcomesKeys.D3GOOD.value], 1)
        self.assertEqual(meas.measurement_data[m.OutcomesKeys.D3FAIR.value], 0)
        self.assertEqual(meas.measurement_data[m.OutcomesKeys.D3POOR.value], 0)
        self.assertEqual(meas.measurement_data[m.OutcomesKeys.ABNORMALFERT.value], -1)
    ...
    def test_type_error_set_patient_information(self):
        ...
        with self.assertRaises(TypeError):
            m.Measurement.set_patient_information(m.PatientKeys.CLINIC, 5.0)
        """ Test that a TypeError is raised if info is not an int """
        with self.assertRaises(TypeError):
            m.Measurement.set_patient_information(m.PatientKeys.PATIENT_NUMBER, 4.2)
    ...
    def test_type_error_set_outcomes(self):
        ...
        with self.assertRaises(TypeError):
            m.Measurement.set_outcome(m.OutcomesKeys.BLASTGRADE, 5.0)
        """ Test that a TypeError is raised if outcome is not an int """
        with self.assertRaises(TypeError):
            m.Measurement.set_outcome(m.OutcomesKeys.D3FAIR, 4.2)
    ...
    def test_type_error_set_property(self):
        ...
        with self.assertRaises(TypeError):
            m.Measurement.set_property(m.PropertyKeys.VIDEO_FRAMES, 5.0)
        """ Test that a TypeError is raised if property is not a float """
        with self.assertRaises(TypeError):
            m.Measurement.set_property(m.PropertyKeys.APPLIED_PRESSURE, 4)
    ...
    def test_type_error_set_model_parameter(self):
        ...
        with self.assertRaises(TypeError):
            m.Measurement.set_property(m.ParameterKeys.ETA0_ZP, 4)
